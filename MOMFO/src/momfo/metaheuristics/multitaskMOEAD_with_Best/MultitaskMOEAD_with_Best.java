...
package momfo.metaheuristics.multitaskMOEAD_with_Best;
...
public class MultitaskMOEAD_with_Best extends Algorithm {


	public MultitaskMOEAD_with_Best(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public MultitaskMOEAD_with_Best(Problem problem) {
		super(problem);
	} // DMOEA
...
//			fileSubscription. printToFile(directoryname.replace("Task1", "Task"+String.valueOf(t+1)) + "/Animation/"+"Dat"+time+".dat",igdHistory.get(t));
	...
	private Solution findBestSolution(int taskNumberToProduce, Integer taskNumberToHelp, int n) throws JMException {
		...
		int Updatetimes = 0;
		int size = populationSize_[taskNumberToHelp];
		...
		double [] ref = new double[ReferencePoint_[taskNumberToProduce].length];
