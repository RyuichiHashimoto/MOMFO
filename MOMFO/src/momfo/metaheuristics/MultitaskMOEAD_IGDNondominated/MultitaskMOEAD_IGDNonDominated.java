...
package momfo.metaheuristics.MultitaskMOEAD_IGDNondominated;
...
public class MultitaskMOEAD_IGDNonDominated extends Algorithm {


	public MultitaskMOEAD_IGDNonDominated(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public MultitaskMOEAD_IGDNonDominated(Problem problem) {
		super(problem);
	} // DMOEA
	...
	public Population execute() throws JMException, ClassNotFoundException {
		...
		for(int t = 0; t< problemSet_.countProblem();t++){
			...
			rank.setPop(population_[t]);
			rank.Ranking();

			boolean[] calc = new boolean[rank.get(0).size()]; //IGD計算をする個体の選出するための配列
			//SkillFactorの照合
