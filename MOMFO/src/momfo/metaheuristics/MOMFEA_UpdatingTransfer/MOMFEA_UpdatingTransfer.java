...
package momfo.metaheuristics.MOMFEA_UpdatingTransfer;
...
public class MOMFEA_UpdatingTransfer extends Algorithm {
	...
	public MOMFEA_UpdatingTransfer(Problem problem) {
		super(problem);
	} // DMOEA

	public MOMFEA_UpdatingTransfer(ProblemSet problemSet_) {
		super(problemSet_);
	...
	public boolean makeNextGeneration() throws JMException{
		...
		for (int p = 0; p < (populationSize_ / 2); p++) {
			...
			if(rand>=rmp){
				...
				} while(parents[1].getSkillFactor() == parents[0].getSkillFactor());
			}
