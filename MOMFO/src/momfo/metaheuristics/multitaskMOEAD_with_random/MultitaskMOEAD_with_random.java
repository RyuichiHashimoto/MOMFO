...
package momfo.metaheuristics.multitaskMOEAD_with_random;
...
public class MultitaskMOEAD_with_random extends Algorithm {


	public MultitaskMOEAD_with_random(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public MultitaskMOEAD_with_random(Problem problem) {
		super(problem);
	} // DMOEA
				...
				for (int i = 0; i < populationSize_[t]; i++) {
					...
					for(int k=0;k<numberOfParents_[tasks];k++){
						...
						if(TaskNumber_List.get(k) == tasks){
							...
							parents[k] = new Solution(problemSet_, tasks); //population_[TaskNumber_List.get(k)].get(parentsNumber.get(k));
						}
					}
