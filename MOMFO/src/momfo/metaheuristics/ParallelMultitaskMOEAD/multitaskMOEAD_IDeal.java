...
public class multitaskMOEAD_IDeal extends Algorithm {


	public multitaskMOEAD_IDeal(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public multitaskMOEAD_IDeal(Problem problem) {
		super(problem);
	} // DMOEA
				...
				for (int i = 0; i < populationSize_[t]; i++) {
					...
					for(int k=0;k<numberOfParents_[tasks];k++){
						if(parentsNumber.get(k) == -1){
							int obj = Random.nextIntIE( problemSet_.get(TaskNumber_List.get(k)).getNumberOfObjectives());
							parents[k] = indArray_[TaskNumber_List.get(k)][obj];
						} else {
