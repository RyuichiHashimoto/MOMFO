...
package momfo.metaheuristics.ParallelMultitaskMOEAD;
...
public class multitaskMOEAD_nonDominated extends Algorithm {


	public multitaskMOEAD_nonDominated(ProblemSet d) {
		super(d);
	...
	List<List<Solution>> nonDominatedSolution;
	 ...
	 *            Problem to solve
	 */
	public multitaskMOEAD_nonDominated(Problem problem) {
		super(problem);
	} // DMOEA
		...
		do {
			...
			for(int t= 0;t < problemSet_.countProblem();t++){
				for (int i = 0; i < populationSize_[t]; i++) {
					...
					matingSelection(parentsNumber,TaskNumber_List,tasks, n, numberOfParents_[tasks]);

					Solution[] parents = new Solution[numberOfParents_[tasks]];

					for(int k=0;k<numberOfParents_[tasks];k++){
						if(parentsNumber.get(k) == -1){
							NDSRanking rank = new NDSRanking(isMAX_);
							//rank.setPop(new Population(population_[TaskNumber_List.get(k)]));
							rank.setPop((population_[TaskNumber_List.get(k)]));
							rank.Ranking();
							int ID = Random.nextIntIE(rank.get(0).size());
							parents[k] = rank.get(0).get(ID);
						}else {
...
//				Solution newSolution = new Solution(problemSet_.get(t));
 ...
 // initPopulation
	...
	public void matingSelection(Vector<Integer> pop_list,Vector<Integer> task_list, int tasknumber,int cid, int size){
		int ss;
		int r;
		int p;
		int taskID;
...
//		System.out.println(sizeOfMatingNeiborhood_ + " 	");
		...
		while (pop_list.size() < size) {
			...
			if(Random.nextDoubleIE() < rmp && counter == 0){
				counter++;
				//他タスクを解く個体との交叉(選択されたベクトル群全体から個体を取得)
				//必ず一方は自分のタスクの個体から選択
				do {
		...
		//		p = neighborhood_[taskID][cid][r];
			...
			for (int i = 0; i < pop_list.size(); i++) {
				if (pop_list.get(i) == p && task_list.get(i) == taskID) // p is in the list
				{
					flag = false;
					break;
				}
...
*/
	...
	void updateProblem(Solution indiv, int Tasks_id,int id) throws JMException {
		int size;
