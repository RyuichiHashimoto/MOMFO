...
public class ParallelMultitaskMOEAD extends Algorithm {


	public  ParallelMultitaskMOEAD(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public ParallelMultitaskMOEAD(Problem problem) {
		super(problem);
	} // DMOEA
		...
		do {
			...
			for(int t= 0;t < problemSet_.countProblem();t++){
				...
				for (int i = 0; i < populationSize_[t]; i++) {
					...
					offSpring_[tasks].add(offSpring);
	...
	//					System.out.println(ParentsSelectioncounter[tasks] + "	");
			...
			for(int t= 0;t < problemSet_.countProblem();t++){
				for (int p = 0; p < populationSize_[t]; p++) {
					updateReference(offSpring_[t].get(p),t);
				}
			}
			for(int t= 0;t < problemSet_.countProblem();t++){
				for (int p = 0; p < populationSize_[t]; p++) {
					updateProblem(offSpring_[t].get(p), t,p);
				}
			}
...
//		maxEvaluations = ((Integer) this.getInputParameter("maxEvaluations")).intValue();
		...
		offSpring_ = new Population[numberOfTasks];
		...
		for(int t =0;t < numberOfTasks;t++){
			...
			offSpring_[t] = new Population(populationSize_[t]);
