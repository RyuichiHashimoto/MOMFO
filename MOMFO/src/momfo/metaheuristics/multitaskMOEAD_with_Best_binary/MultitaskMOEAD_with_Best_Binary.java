...
package momfo.metaheuristics.multitaskMOEAD_with_Best_binary;
...
public class MultitaskMOEAD_with_Best_Binary extends Algorithm {


	public MultitaskMOEAD_with_Best_Binary(ProblemSet d) {
		super(d);
	 ...
	 *            Problem to solve
	 */
	public MultitaskMOEAD_with_Best_Binary(Problem problem) {
		super(problem);
	} // DMOEA
			...
			for(int t= 0;t < problemSet_.countProblem();t++){
				for (int i = 0; i < populationSize_[t]; i++) {
					...
					for(int k=0;k<numberOfParents_[tasks];k++){
						//タスク番号
						if(TaskNumber_List.get(k) == tasks){
							parents[k] = population_[TaskNumber_List.get(k)].get(parentsNumber.get(k));
						} else {
							parents[k] = findBestSolution(tasks, TaskNumber_List.get(k),n); //new Solution(problemSet_, tasks); //population_[TaskNumber_List.get(k)].get(parentsNumber.get(k));
						}
					}
...
//			fileSubscription. printToFile(directoryname.replace("Task1", "Task"+String.valueOf(t+1)) + "/Animation/"+"Dat"+time+".dat",igdHistory.get(t));
	...
	private Solution findBestSolution(int taskNumberToProduce, Integer taskNumberToHelp, int n) throws JMException {

		int size = binarySize;//populationSize_[taskNumberToHelp];
		int[] perm = new int[size];

		// generate teh random permutation.
		Permutation.randomPermutation(perm, size);
		int bestIndIndex = perm[0];
		Solution bestsol = new Solution(population_[taskNumberToHelp].get(perm[0]));
		problemSet_.get(taskNumberToProduce).evaluate(bestsol);
		double [] ref = new double[bestsol.getNumberOfObjectives()];

		//一時的な参照点の更新
		for(int i   = 0; i < ref.length;i++){
			ref[i] = Math.min(ReferencePoint_[taskNumberToProduce][i], bestsol.getObjective(i));
		}


		for (int i = 1; i < size; i++) {
			int p = perm[i];
			Solution sol = new Solution(population_[taskNumberToHelp].get(p));
			problemSet_.get(taskNumberToProduce).evaluate(sol);
			comparator[taskNumberToProduce].setWeightedVector(WeightedVector_[taskNumberToProduce][n]);
			for(int r   = 0; r < ref.length;r++){
				ref[r] = Math.min(ref[r], sol.getObjective(r));
			}

			comparator[taskNumberToProduce].setRefernecePoint(ref);
			if (comparator[taskNumberToProduce].execute(sol, bestsol) == 1) {
				bestIndIndex = p;
				sol =  bestsol;
			}
		}

		return population_[taskNumberToHelp].get(bestIndIndex);
...
//		maxEvaluations = ((Integer) this.getInputParameter("maxEvaluations")).intValue();
		...
		binarySize = ((int)  this.getInputParameter("BinarySize"));
		for(int t =0;t < numberOfTasks;t++){
