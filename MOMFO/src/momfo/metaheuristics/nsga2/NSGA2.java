...
public class NSGA2 extends GeneticAlgorithm {
	...
	public void initialize(int seed) throws ClassNotFoundException, JMException {
		...
		comparator_binary = new NSGAIIComparatorBinary(isMAX_, random);
		...
		ranking = null;
		igd[0] = counter;
		igd[1] = (IGD.CalcNormalizeIGD_To_NonDominated(population_.getAllObjectives(),
	...
	public void nextGeneration() throws JMException {
		...
		if (populationSize_ != population_.size()) {
			...
			assert d.size() + population_.size() == populationSize_ : "たのしいな" + ret.size() + "	" + population_.size()
					+ "	" + populationSize_;
		...
		ranki_.Ranking();
		double igd = (IGD.CalcNormalizeIGD_To_NonDominated(population_.getAllObjectives(),
				IGDRef.getNormalizeRefs(tasknumber), IGDRef.getMaxValue(tasknumber), IGDRef.getMinValue(tasknumber),
				random));
