...
public class MMZDT extends Problem {
	...
	public MMZDT(String function,int numberOfVariables, int k, double lg, double ug) {
		...
		for (int var = 0; var < numberOfVariables - k_; var++) {
			lowerLimit_[var] = lg;
			upperLimit_[var] = ug;
		} // for
		for (int var = numberOfVariables - k_; var < numberOfVariables; var++) {
			lowerLimit_[var] = 0.0;
			upperLimit_[var] = 1.0;
		}
	...
	public void evaluate(Solution solution) throws JMException {
		...
		double[] xI = new double[numberOfVariables_ - k_];
		double[] xII = new double[k_];
		for (int i = 0; i < numberOfVariables_ - k_; i++)
			xI[i] = vars[i];

		for (int i = numberOfVariables_ - k_; i < numberOfVariables_; i++)
			xII[i - (numberOfVariables_ - k_)] = vars[i];



	//	xII = transformVariables(xII);

		double f1 = evalF1(xII);
		double g = gfunction.evaluate(xI)+1.0;
