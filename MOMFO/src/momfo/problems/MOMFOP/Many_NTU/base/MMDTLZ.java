package momfo.problems.MOMFOP.Many_NTU.base;
...
public class MMDTLZ extends Problem {
	...
	public MMDTLZ(String function,int numberOfObjectives, int numberOfVariables, int alpha, double lg, double ug) {
		...
		for(int var = 0; var < numberOfVariables - numberOfObjectives_ +1;var++){
			lowerLimit_[var] = lg;
			upperLimit_[var] = ug;
		}

		for (int var =  numberOfVariables - numberOfObjectives_ +1; var < numberOfVariables; var++) {
			lowerLimit_[var] = 0.0;
			upperLimit_[var] = 1.0;
		} // for
...
//		double vars[] = scaleVariables(solution);
		...
		double[] xI = new double[numberOfVariables_  - numberOfObjectives_ +1];
		double[] xII = new double[numberOfObjectives_ -1];

		for (int i = 0; i < numberOfVariables_  - numberOfObjectives_ +1; i++)
			xI[i] = vars[i];

		for (int i =numberOfVariables_  - numberOfObjectives_ +1; i < numberOfVariables_; i++)
			xII[i - (numberOfVariables_  - numberOfObjectives_ +1)] = vars[i];


		double[] f = new double[numberOfObjectives_];

		double g = gfunction.evaluate(xI);
		for (int i = 0; i < numberOfObjectives_; i++)
			f[i] = 1 + g;

		for (int i = 0; i < numberOfObjectives_; i++) {
			for (int j = 0; j < numberOfObjectives_ - (i + 1); j++)
				f[i] *= Math.cos(Math.pow(xII[j], alpha_) * 0.5 * Math.PI);
			if (i != 0) {
				int aux = numberOfObjectives_ - (i + 1);
				f[i] *= Math.sin(Math.pow(xII[aux], alpha_) * 0.5 * Math.PI);
			} // if
		} // for
