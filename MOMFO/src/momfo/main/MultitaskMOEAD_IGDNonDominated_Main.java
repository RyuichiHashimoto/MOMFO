...
import momfo.metaheuristics.MultitaskMOEAD.MultitaskMOEAD;
...
public class MultitaskMOEAD_IGDNonDominated_Main extends AlgorithmMain{

	public MultitaskMOEAD_IGDNonDominated_Main(Setting test) {
		super(test);
	}
	...
	//	algorithm = new GA[2];
		...
		algorithm = new MultitaskMOEAD(problemSet_);
		for (int t=0;t < problemSet_.countProblem();t++){
			...
			if(setting_.getAsStr("ScalarFunction").split(",")[t].startsWith("PBI")){
				DirectoryName = "result/MultitaskMOEAD/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t]+setting_.getAsStr("PBITheta").split(",")[t].replace(".","_");
			}else {
				DirectoryName = "result/MultitaskMOEAD/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t];
			}
