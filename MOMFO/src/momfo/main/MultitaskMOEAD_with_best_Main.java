...
import momfo.metaheuristics.multitaskMOEAD_with_Best.MultitaskMOEAD_with_Best;
...
public class MultitaskMOEAD_with_best_Main extends AlgorithmMain{

	public MultitaskMOEAD_with_best_Main(Setting test) {
		super(test);
	}
	...
	//	algorithm = new Algorithm[2];
		...
		algorithm = new MultitaskMOEAD_with_Best(problemSet_);
		for (int t=0;t < problemSet_.countProblem();t++){
			...
			if(setting_.getAsStr("ScalarFunction").split(",")[t].startsWith("PBI")){
				DirectoryName = "result/MultitaskMOEAD_with_best/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t]+setting_.getAsStr("PBITheta").split(",")[t].replace(".","_");
			}else {
				DirectoryName = "result/MultitaskMOEAD_with_best/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t];
			}
