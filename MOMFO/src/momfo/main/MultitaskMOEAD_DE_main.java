...
import momfo.metaheuristics.MultitaskMOEAD_DE.MultitaskMOEAD_DE;
...
public class MultitaskMOEAD_DE_main extends AlgorithmMain{

	public MultitaskMOEAD_DE_main(Setting test) {
		super(test);
	}
	...
	//	algorithm = new GA[2];
		...
		algorithm = new MultitaskMOEAD_DE(problemSet_);
		for (int t=0;t < problemSet_.countProblem();t++){
			...
			if(setting_.getAsStr("ScalarFunction").split(",")[t].startsWith("PBI")){
				DirectoryName = "result/MultitaskMOEADDE/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t]+setting_.getAsStr("PBITheta").split(",")[t].replace(".","_");
			}else {
				DirectoryName = "result/MultitaskMOEADDE/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t];
			}
