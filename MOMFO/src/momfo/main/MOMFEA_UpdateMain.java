...
import momfo.metaheuristics.MOMFEA_UpdatingTransfer.MOMFEA_UpdatingTransfer;
...
public class MOMFEA_UpdateMain extends AlgorithmMain{

	public MOMFEA_UpdateMain(Setting test) {
		super(test);
	}
...
//		int OBJ = setting_.getAsInt("Objectives");
		...
		//	continue;
		//}
		algorithm = new MOMFEA_UpdatingTransfer(problemSet_);
		int OBJ = problemSet_.get(t).getNumberOfObjectives();

		String dddname = setting_.getAsStr("rmp") + "rmp"+problemSet_.get(t).getNumberOfObjectives()  + "OBJ";
		algorithm.setInputParameter("populationSize", setting_.getAsInt("populationSize"));

		DirectoryName = "result/MOMFEA_UP/" + Problemname+"/Task"+String.valueOf(t+1)+"/"+dddname;
