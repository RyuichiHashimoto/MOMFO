...
public class ParallelMultitaskMOEAD_Main extends AlgorithmMain{

	public ParallelMultitaskMOEAD_Main(Setting test) {
		super(test);
	}
	...
	//	algorithm = new Algorithm[2];
		...
		for (int t=0;t < problemSet_.countProblem();t++){
			...
			if(setting_.getAsStr("ScalarFunction").split(",")[t].startsWith("PBI")){
				DirectoryName = "result/ParallelMultitaskMOEAD/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t]+setting_.getAsStr("PBITheta").split(",")[t].replace(".","_");
			}else {
				DirectoryName = "result/ParallelMultitaskMOEAD/" + Problemname+"/"+"Task"+String.valueOf(1)+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction").split(",")[t];
			}
